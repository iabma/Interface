...
document.getElementById("add-custom").addEventListener("click", () => {
    ...
    if (document.getElementById("template").innerHTML == "Use the search bar to begin.")
        return;
    var templateOutput = [];
...
function createTemplateVisual(content, variables) {
    ...
    content.forEach(element => {
        if (element.type == "text") {
            ...
            document.getElementById("template").innerHTML += addDropdown(element.name, element.content, 1, variables);
        }
    });
    updateDropdownAnim();
}

function addProgression(content, depth, variables) {
    ...
    content.forEach(element => {
        if (element.type == "text") {
            ...
            progression.innerHTML += addDropdown(element.name, element.content, depth + 1, variables);
        }
    });
    stylize(progression);
...
function addDropdown(name, options, depth, variables) {
    //console.log(variables[0])
    ...
    if (variables != null && variables[0]) {
        let val = variables.shift();
        button.innerHTML = val.type == "text" ? val.content == "" ? "<span>empty</span>" : "<span>" + val.content + "</span>" : val.type == "progression" ? addProgression(val.content, depth + 1, variables) : addDropdown(val.name, val.content, depth + 1, variables);
    } else if (options[0])
    ...
    options.forEach(option => {
        ...
        if (option.type == "text") {
            ...
            opt.innerHTML = addProgression(option.content, depth + 1, variables);
        } else if (option.type == "dropdown") {
            opt.innerHTML = addDropdown(option.name, option.content, depth + 1, variables);
        }
        stylize(opt);
...
window.onmessage = e => {
    if (e.data[0] == "openEmptyTemplate") {
        createTemplateVisual(getTemplate(e.data[1]).English.content);
    } else {
        let name = e.data[0],
            variables = e.data[1];
    
        createTemplateVisual(getTemplate(name).English.content, variables);
    }
};